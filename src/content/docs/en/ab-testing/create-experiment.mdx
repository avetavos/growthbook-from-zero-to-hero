---
title: 'Create an Experiment'
description: 'Step-by-step: create an A/B experiment in the GrowthBook UI, define variations, set a traffic split, choose metrics, and start the test.'
sidebar:
  order: 2
---

import CopyCode from '../../../../components/CopyCode.tsx';
import Callout from '../../../../components/Callout.astro';
import Quiz from '../../../../components/Quiz.tsx';
import ProgressTracker from '../../../../components/ProgressTracker.tsx';

## Creating an A/B Experiment in GrowthBook

GrowthBook lets you run experiments either as standalone **Experiments** or as **Experiment rules** attached to a Feature flag. Both paths converge on the same experiment editor — the steps below cover the most common path: adding an Experiment rule to an existing feature flag.

### Step 1 — Open the Features list and choose a feature

1. Log in to GrowthBook at `http://localhost:3000` (or your Cloud URL).
2. In the left sidebar, click **Features**.
3. Click the feature you want to experiment on — for example, `checkout-experiment`.

### Step 2 — Add an Experiment rule

1. On the Feature detail page, locate the environment where you want to run the test (e.g. **production**).
2. Click **Add Rule** and select **Experiment** from the rule-type dropdown.
3. The experiment editor opens inline.

### Step 3 — Define your variations

1. Under **Variations**, you will see a **Control** row already present.
2. Click **Add Variation** to create one or more treatment variants.
3. Set the **Value** for each variation. For a string feature this is the value the SDK returns — e.g. `'control'` and `'variant'`.
4. Give each variation a human-readable **Name** so reports are readable.

### Step 4 — Set the traffic split

1. In the **Traffic** section, set the **percentage of users** included in the experiment (e.g. `50%` to expose half your traffic).
2. Adjust the **weights** between variations. Equal weights (`50 / 50`) are the default and recommended starting point.
3. GrowthBook uses a deterministic hash of the user ID to assign buckets — the same user always sees the same variation.

### Step 5 — Choose your metrics

1. Scroll to the **Metrics** section.
2. Click **Add Metric** and search for your primary metric (e.g. `checkout_completed`).
3. Add any secondary or guardrail metrics you want to monitor.
4. Mark exactly one metric as the **Primary metric** — this is the single source of truth for your decision.

### Step 6 — Start the experiment

1. Review the **Hypothesis** field and type a falsifiable statement, e.g. *"Showing the new checkout flow will increase checkout_completed by 5%."*
2. Click **Save Rule**, then toggle the environment from **Draft** to **Running**.
3. GrowthBook begins collecting impression and conversion data immediately.

## Checking the variation in your code

After the experiment is live, use `getFeatureValue` to read the assigned variation. Branch your logic on the returned string:

export const createExperimentCode = `import { GrowthBook } from '@growthbook/growthbook';

const gb = new GrowthBook({
  apiHost: 'https://cdn.growthbook.io',
  clientKey: 'sdk-YOUR_DEV_KEY',
  trackingCallback: (experiment, result) => {
    // Send impression to your analytics pipeline
    console.log('Experiment:', experiment.key, 'Variation:', result.variationId);
  },
});

await gb.init({ timeout: 2000 });

const variation = gb.getFeatureValue('checkout-experiment', 'control');

if (variation === 'variant') {
  // Show the new checkout flow
  renderNewCheckout();
} else {
  // Default: show the original checkout flow
  renderOriginalCheckout();
}`;

<CopyCode client:visible code={createExperimentCode} />

<Callout title="Key point">
Start with a clear hypothesis and one primary metric. Defining the primary metric before you start prevents HARKing (Hypothesizing After Results are Known) and keeps your statistical analysis honest.
</Callout>

## Trade-offs

| Option | Benefit | Cost |
|---|---|---|
| Experiment with a clear hypothesis defined upfront | Measures the right thing, easier to decide on | Requires time to think through the hypothesis before starting |
| Experiment that just "tries something" with no hypothesis | Starts faster | Risks p-hacking — finding a fake pattern after the fact |

## Common mistakes

- Starting an experiment without a clear hypothesis
- Testing several variations at once by changing multiple things in one variant (so you can't tell what actually drove the result)
- Ending an experiment too early, before reaching the planned sample size

> 💡 Real-world example
>
> Teams that write down a falsifiable hypothesis before every experiment reduce the risk of interpreting ambiguous results to fit a preferred conclusion after the fact.

export const quizCreateExperiment = [
  {
    q: 'Where in the GrowthBook UI do you create an experiment?',
    options: [
      'Settings → SDK Connections → Add Experiment',
      'Features → select a feature → Add Rule → Experiment (or Experiments → Add Experiment)',
      'Metrics → Add Metric → Attach Experiment',
      'Data Sources → Run Query',
    ],
    answer: 1,
  },
  {
    q: 'What does "traffic split" mean in the context of a GrowthBook experiment?',
    options: [
      'The ratio of API requests routed to different servers',
      'The percentage of users included in the experiment and how that traffic is divided among variations',
      'The number of metrics tracked per variation',
      'The time window during which the experiment runs',
    ],
    answer: 1,
  },
  {
    q: 'Why should you define a primary metric before starting an experiment?',
    options: [
      'GrowthBook will not save the experiment without it',
      'It prevents choosing the winning metric after seeing results, keeping the analysis statistically valid',
      'It automatically pauses the experiment when the metric goal is reached',
      'It determines which SDK method to use in your code',
    ],
    answer: 1,
  },
];

<Quiz client:visible id="ab-testing/create-experiment" questions={quizCreateExperiment} />

<ProgressTracker client:visible id="ab-testing/create-experiment" />
