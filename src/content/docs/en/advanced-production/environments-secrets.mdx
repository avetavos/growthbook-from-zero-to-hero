...
| Option | Benefit | Cost |
|---|---|---|
| Store secrets in an environment variable / secret manager | Safer, kept separate from code | More infrastructure to set up (secret manager, injection) |
| Hardcode in a config file | Faster during development | Risk of the secret leaking into the git repository |

## Common mistakes

- Committing an API secret or SDK secret key into the git repository
- Reusing the same secret across environments, so revoking one key breaks everything
- Never rotating secrets

> 💡 Real-world example
>
> Security teams at larger companies require every secret to go through a secret manager (e.g. AWS Secrets Manager, Vault) rather than a plain `.env` file for production.
