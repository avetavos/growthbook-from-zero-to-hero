---
title: 'REST API & Webhooks'
description: 'Use the GrowthBook REST API to manage flags programmatically and webhooks to propagate changes in real time.'
sidebar:
  order: 4
---

import CopyCode from '../../../../components/CopyCode.tsx';
import Callout from '../../../../components/Callout.astro';
import Quiz from '../../../../components/Quiz.tsx';
import ProgressTracker from '../../../../components/ProgressTracker.tsx';

## The GrowthBook REST API

GrowthBook exposes a REST API that lets you read and manage features, experiments, metrics, and more — programmatically. This is the foundation for CI/CD automation: your deployment pipeline can toggle flags, check experiment status, or create new features without anyone opening the UI.

The base URL for self-hosted installs is your GrowthBook app URL, e.g. `http://localhost:3100/api/v1`.

### Step 1 — Create an API key

1. In GrowthBook, go to **Settings → API Keys**.
2. Click **Add API Key**.
3. Choose **Secret Key** (full read/write access) or **Read-only Key** (safe for dashboards).
4. Give it a description, e.g. `ci-pipeline`.
5. Click **Create** and copy the key — it is shown only once.

### Step 2 — List features via curl

export const listFeaturesCode = `curl -s \\
  -H "Authorization: Bearer YOUR_API_KEY" \\
  http://localhost:3100/api/v1/features | \\
  jq '.features[] | {id, defaultValue}'`;

<CopyCode client:visible code={listFeaturesCode} label="Copy list-features curl" />

The response is a JSON object. The `.features` array contains every feature with its key, environments, and rules.

### Step 3 — Toggle a feature on/off via curl

export const toggleFeatureCode = `# Enable 'checkout-v2' in the production environment
curl -s -X POST \\
  -H "Authorization: Bearer YOUR_API_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{"environments": {"production": true}}' \\
  http://localhost:3100/api/v1/features/checkout-v2/toggles`;

<CopyCode client:visible code={toggleFeatureCode} label="Copy toggle-feature curl" />

Replace `YOUR_API_KEY` with the secret key you created in Step 1, and `checkout-v2` with your feature key.

## SDK Webhooks

Webhooks are outgoing HTTP calls that GrowthBook fires every time a feature or experiment changes. The primary use case is **SDK cache invalidation**: your CDN or application server receives the webhook and immediately re-fetches the GrowthBook feature payload, so users get the latest flag state without waiting for the next poll cycle.

### Creating an SDK webhook

1. Go to **Settings → SDK Connections** and open the SDK connection you want to hook.
2. Click **Webhooks** → **Add Webhook**.
3. Enter the **Endpoint URL** — the HTTPS URL your server or CDN listens on.
4. (Optional) Set a **Signing Secret** so your server can verify the payload came from GrowthBook.
5. Click **Save**. GrowthBook sends a test ping immediately.

### Verifying the webhook signature

GrowthBook signs the webhook body with HMAC-SHA256. On your server:

```js
import crypto from 'node:crypto';

function verifyWebhook(body, signature, secret) {
  const expected = crypto
    .createHmac('sha256', secret)
    .update(body)
    .digest('hex');
  return crypto.timingSafeEqual(
    Buffer.from(expected),
    Buffer.from(signature)
  );
}
```

The `X-GrowthBook-Signature` header contains the hex digest.

## Event webhooks

Beyond SDK cache invalidation, GrowthBook also supports **event webhooks** — notifications for higher-level events such as experiment started, experiment stopped, or feature saved. Configure these under **Settings → Event Webhooks**.

export const eventWebhookPayload = `// Example GrowthBook event webhook payload (feature.updated)
{
  "event": "feature.updated",
  "data": {
    "id": "checkout-v2",
    "environments": {
      "production": { "enabled": true }
    }
  },
  "timestamp": "2025-09-01T12:00:00Z"
}`;

<CopyCode client:visible code={eventWebhookPayload} label="Copy example webhook payload" />

<Callout title="Key point">
Use the REST API and webhooks together to automate flag changes from CI/CD — the API toggles the flag, and the webhook tells your servers to invalidate their SDK cache immediately.
</Callout>

## Trade-offs

| Option | Benefit | Cost |
|---|---|---|
| Webhook | Real-time notification when a flag changes, good for automation | You have to handle retries/signature verification yourself |
| Polling via the API | Simpler | Adds delay and wastes requests |

## Common mistakes

- Not verifying the webhook signature, risking forged requests
- Not handling retries when the webhook endpoint is down, missing events
- Storing the API key in client-side code that is reachable externally

> 💡 Real-world example
>
> DevOps teams commonly wire GrowthBook webhooks to Slack to get an immediate alert whenever someone changes a flag in production, which speeds up auditing and debugging.

export const quizWebhooksApi = [
  {
    q: 'Where in GrowthBook do you create an API key?',
    options: [
      'Features → Add Feature',
      'Settings → API Keys',
      'Experiments → Metrics',
      'SDK Connections → Webhooks',
    ],
    answer: 1,
  },
  {
    q: 'What HTTP method does the GrowthBook toggle endpoint use?',
    options: [
      'GET',
      'POST',
      'PUT',
      'PATCH',
    ],
    answer: 1,
  },
  {
    q: 'What is the primary use case for an SDK webhook?',
    options: [
      'To run statistical analysis on experiment results',
      'To invalidate the SDK feature payload cache when flags change',
      'To create new features automatically',
      'To authenticate SDK connections',
    ],
    answer: 1,
  },
  {
    q: 'Which HTTP header does GrowthBook use to send the HMAC-SHA256 webhook signature?',
    options: [
      'X-GrowthBook-Token',
      'X-GrowthBook-Signature',
      'Authorization',
      'X-Webhook-Secret',
    ],
    answer: 1,
  },
];

<Quiz client:visible id="advanced-production/webhooks-api" questions={quizWebhooksApi} />

<ProgressTracker client:visible id="advanced-production/webhooks-api" />
